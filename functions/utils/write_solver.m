function write_solver(conf)
    ...
    tempname = lower(conf.solver_type);
    
    if safeDefault(conf, 'use_poly')
        tempname = [tempname '_poly'];
    end

    solver_txt = fileread([conf.base_dir '/models/solver_template_' tempname '.prototxt']);
