function [rois] = get_top_proposals(conf, rpn_net, roidb, imdb, rcnn_conf)
    ...
    if safeDefault(rcnn_conf, 'custom_feat_cls_layer')
       conf.custom_feat_cls_layer = rcnn_conf.custom_feat_cls_layer;
    end
    if safeDefault(rcnn_conf, 'custom_cls_layer')
       conf.custom_cls_layer = rcnn_conf.custom_cls_layer;
    end
    if safeDefault(rcnn_conf, 'custom_bbox_layer')
      conf.custom_bbox_layer = rcnn_conf.custom_bbox_layer;
    end
    ...
    for i=1:im_count
        ...
        % filter rpn
        proposal_num = 40; %rcnn_conf.train_batch_size;
        assert(proposal_num>=rcnn_conf.train_batch_size);
        ...
        if safeDefault(rcnn_conf, 'concat_radial_grad')
            
            auto_data = get_radial(im, conf, rcnn_conf, boxes, rpn_net);
            rois{i}.auto_data = auto_data;
        end
