...
objs=set(objs,'bb',(get(objs,'bb')));
objs=set(objs,'bbv',(get(objs,'bbv')));
objs=set(objs,'ang',(get(objs,'ang')));
for i=1:length(objs)
  o=objs(i); bb=o.bb; bbv=o.bbv;
  
  % gb changed now doubles
  %fprintf(fid,['%s' repmat(' %i',1,11) '\n'],o.lbl, bb,o.occ,bbv,o.ign,o.ang);
  fprintf(fid,['%s' repmat(' %0.6f',1,11) '\n'],single(o.lbl), single(bb),single(o.occ),single(bbv),single(o.ign),single(o.ang));
end
fclose(fid);
end
...
if( format==0 )
  ...
  % gb changed made doubles
  %frmt='%s %d %d %d %d %d %d %d %d %d %d %d';
  frmt='%s %f %f %f %f %f %f %f %f %f %f %f';
